...
contract CustomTokyoFizz is ERC721 {
    ...
    function mint(uint256 amount) internal payable {
    ...
    function newAdmin(address a) public requires DEFAULT_ADMIN_ROLE {
        _setupRole(DEFAULT_ADMIN_ROLE, a);
    }

    function pause() public requires DEFAULT_ADMIN_ROLE {
        _pause();
    }

    function unpause() public requires DEFAULT_ADMIN_ROLE {
        _unpause();
    }

    function transferBalence(uint256 amount)
        public
        payable
        requires
        DEFAULT_ADMIN_ROLE
